...
#        dy=ypool.dot(Xi)
    ...
    def GenerateRobotSINDYcModel(self,
        ...
        if discrete_model is True:
            ...
            x_robot_tof_train=self.DataDictionary['x_train'][:,0:N_states]
...
try:
    ...
    # =============================================================================
    RoSEv2pt0_Obj_SINDYc_SI.DesignSINDYcModelForMPC(threshold_vec=np.array([0.00004]))
    ...
    for i in range(Nvec.shape[0]):
        ...
        Duration    = 680#94.9#354#94.9#530             # Run control for 100 time units
    ...
    #    D = 0                          # Feedforward (none)
        ...
        xref_df_all=pd.read_csv('ControllerReferenceFiles/Reference_traj_tof_for_FSP_testing_2.csv')
        xref_all=np.array(xref_df_all)[:,0:]
        xref0 = 0*np.ones((1,300))#int(Ton/Ts))) #Initial part of the reference where the control is off
        #xref1 = xref_all[0:1100,1:].T
        
        
        xref_val=np.array([88,90,92,94,96,98,100,98,96,94,92,90,88])
        xref_rep=np.repeat(xref_val,500)
        xref_rep_cycle=np.array([(np.ones((1,1))*xref_rep).flatten()]).T
...
#                pdb.set_trace()
                if xref[0]<88:
                    LB1=6+6 #Lower bound
                    UB1=12+6 #Upper bound
                elif xref[0]>=88 and xref[0]<91:
                    LB1=14 #Lower bound
                    UB1=16 #Upper bound
                elif xref[0]>=91 and xref[0]<94:
                    LB1=17
                    UB1=19
                elif xref[0]>=94 and xref[0]<97:
                    LB1=20
                    UB1=22
                elif xref[0]>=97 and xref[0]<99:
                    LB1=23
                    UB1=25 
                elif xref[0]>=99 and xref[0]<102:
                    LB1=26
                    UB1=28
                elif xref[0]>=102 and xref[0]<105:
                    LB1=29
                    UB1=31
                elif xref[0]>=105 and xref[0]<109:
                    LB1=36
                    UB1=37
...
#                        UB1=121#92.5
            ...
            x = np.array([[range_mm_array_filtered[2]]])+11
            x_adc=np.array([[pressure_kpa_array[1]]])

            
#            t_History=tHistory[-1]+Ts/1
            xHistory=np.concatenate((xHistory,x[:,0]))
            x_adcHistory=np.concatenate((x_adcHistory,x_adc[:,0]))
        ...
        HistoryDict={
                  't':tHistory,
                  'x_ref_1':1*(rHistory-0),
                  'x_1':1*(xHistory-0),#(5v/256*ADC_data-1)*125
                  'x_adc':(5/256*x_adcHistory-1)*125,
        ...
        History_df.to_csv('DataFiles/Data15_10_2020_stent_FSP_experiment_with_MPC/data_2.csv')
...
#                linestyle='--')
        ...
        ph0 = ax1.plot(HistoryDict['x_1']-3,
                    color='darkblue',
                    linewidth=1.5,
                    linestyle='-',
                    label='Output')
