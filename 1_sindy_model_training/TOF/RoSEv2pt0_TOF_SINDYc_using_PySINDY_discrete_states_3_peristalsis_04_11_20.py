...
RoSEv2pt0_3layer_TOFADCandPer.FileReading([3,0,2,1])
#12,13
...
for c, value in enumerate(dict_keys):
    ...
    """
    layer_index=5
    int_data=(270,270,270,270,170,170)#(1100,1100,1100,1100,170)
    num_of_dt_pts=(1200,1200,1200,1200,8000)#(5000,5000,5000,5000,12000)  
...
x_RoSEv2pt0_3layer_tof_all=np.array(x_tof_online_fil_list)
...
x_RoSEv2pt0_3layer_tof_train=np.array([sf1,sf2,sf3])*(x_RoSEv2pt0_3layer_tof_train-np.array([23,73,69]))+np.array([1,0,1])
x_RoSEv2pt0_3layer_tof_all=np.array([sf1,sf2,sf3])*(x_RoSEv2pt0_3layer_tof_all-np.array([23,73,69]))+np.array([1,0,1])
...
for i in range(threshold_vec.shape[0]):
    ...
    try: 
       ...
       model_discrete.fit(x_RoSEv2pt0_3layer_tof_train[:-1], t=1.0,
                          x_dot=x_RoSEv2pt0_3layer_tof_train[1:], u=u_RoSEv2pt0_3layer_train[:-1])
       ...
       x0_RoSEv2pt0_3layer_tof_all=x_RoSEv2pt0_3layer_tof_all[0,np.newaxis]
       ...
       x_RoSEv2pt0_3layer_tof_all_sim=np.zeros((N_all,3))
       x_RoSEv2pt0_3layer_tof_all_sim[0,:]=x0_RoSEv2pt0_3layer_tof_all
       ...
       for ii in range(1,N_all):
           x_RoSEv2pt0_3layer_tof_all_sim[ii] = model_discrete.predict(x0_RoSEv2pt0_3layer_tof_all, 
                                                      u_RoSEv2pt0_3layer_dummy_test[ii-1:ii])
           # print(x_QSR_DoubleLayer_adc_all_sim[ii])
           x0_RoSEv2pt0_3layer_tof_all=x_RoSEv2pt0_3layer_tof_all_sim[ii,np.newaxis]
       
       #Saving as a csv file for filtered adc
       x_u_conc_fil_tof=np.hstack(( u_RoSEv2pt0_3layer_all,
                                x_RoSEv2pt0_3layer_tof_all
                                ))
       x_u_conc_fil_tof_df=pd.DataFrame(data=x_u_conc_fil_tof,
                                columns=['u0','u1','u2','x_fil0','x_fil1','x_fil2'])
       x_u_conc_fil_tof_df.to_csv('ControllerReferenceFiles/Reference_traj_filtered_tof_1.csv')    
       
       #Saving as a csv filefor simulated
       x_u_conc=np.hstack(( u_RoSEv2pt0_3layer_dummy_test,
                                x_RoSEv2pt0_3layer_tof_all_sim
                                ))
       ...
       for i in range (1,4):
           ax3[i].plot(x_RoSEv2pt0_3layer_tof_all[0:FinPt1,i-1],#/np.max(x_QSR_DoubleLayer_tof_all[0:FinPt]),
                      label='training data',
                      color='green',
                      alpha=0.7)
           ax3[i].plot(np.arange(0,FinPt1),x_RoSEv2pt0_3layer_tof_all_sim[0:FinPt1,i-1],#/np.max(x_QSR_DoubleLayer_tof_all_sim[0:FinPt]),
                           label='model simulation on training data',
                           color='blue',
                           alpha=1)
           ax3[i].plot(np.arange(FinPt1,FinPt2),x_RoSEv2pt0_3layer_tof_all_sim[FinPt1:FinPt2,i-1],#/np.max(x_QSR_DoubleLayer_tof_all_sim[0:FinPt]),
                          'k',label='model simulation on test data',
                          alpha=1)
           ax3[i].plot(np.arange(FinPt1,FinPt3),x_RoSEv2pt0_3layer_tof_all[FinPt1:FinPt3,i-1],#/np.max(x_QSR_DoubleLayer_tof_all_sim[0:FinPt]),
                          'r',label='test data',
                          alpha=0.7)
       ax3[1].set_ylim(-12,10)
       ax3[2].set_ylim(-12,10)
       ax3[3].set_ylim(-12,10)
       plt.show()
