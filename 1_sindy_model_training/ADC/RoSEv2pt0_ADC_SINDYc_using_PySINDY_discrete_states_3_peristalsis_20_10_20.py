...
# from scipy import zeros, signal, random  # removed: zeros/signal/random already imported via numpy/scipy.signal/random
...
FigureNum.suptitle('Raw Sensor Data — ADC 3-State Peristalsis (20/10/20)',
...
for i in range(threshold_vec.shape[0]):
    ...
    try: 
       ...
       model_discrete.fit(x_RoSEv2pt0_3layer_adc_train[:-1], t=1.0,
                          x_dot=x_RoSEv2pt0_3layer_adc_train[1:], u=u_RoSEv2pt0_3layer_train[:-1])
       ...
       FigureNum=plt.figure(num=5+i, figsize=(14, 10))
       FigureNum.suptitle('DTSINDYc Model Validation — ADC 3-State Peristalsis\n'
                          f'(threshold = {threshold_vec[i]:.4f})',
                          fontsize=14, fontweight='bold')
       ax3=[FigureNum.add_subplot(4,1,j) for j in range(1,5)]
       
       ax3[0].plot(np.arange(0,FinPt2),u_RoSEv2pt0_3layer_dummy_test[0:FinPt2],
                 alpha=0.7,
                 label='DAC input (u)')
       ax3[0].set_ylabel('DAC command', fontsize=10)
       ax3[0].legend(loc='upper right', fontsize=9, framealpha=0.9)
       ax3[0].grid(True, alpha=0.3)
       ax3[0].axvline(x=FinPt1, color='grey', linestyle=':', linewidth=1, label='Train/Valid split')
       
       state_names = [r'$x_1$', r'$x_2$', r'$x_3$']
       for j in range (1,4):
           ax3[j].plot(x_RoSEv2pt0_3layer_adc_all[0:FinPt1,j-1],
                      color='tab:blue', linewidth=1.5, label='Measured (train)')
           ax3[j].plot(np.arange(0,FinPt1),x_RoSEv2pt0_3layer_adc_all_sim[0:FinPt1,j-1],
                          color='tab:orange', linewidth=1.5, linestyle='--', label='Model (train)')
           ax3[j].plot(np.arange(FinPt1,FinPt2),x_RoSEv2pt0_3layer_adc_all_sim[FinPt1:FinPt2,j-1],
                          color='black', linewidth=1.5, linestyle='--', label='Model (extrapolated)')
           ax3[j].plot(np.arange(FinPt1,FinPt3),x_RoSEv2pt0_3layer_adc_all[FinPt1:FinPt3,j-1],
                          color='tab:red', linewidth=1.5, label='Measured (valid)')
           ax3[j].axvline(x=FinPt1, color='grey', linestyle=':', linewidth=1)
           ax3[j].set_ylabel(f'State {state_names[j-1]}', fontsize=10)
           ax3[j].legend(loc='upper right', fontsize=8, framealpha=0.9, ncol=2)
