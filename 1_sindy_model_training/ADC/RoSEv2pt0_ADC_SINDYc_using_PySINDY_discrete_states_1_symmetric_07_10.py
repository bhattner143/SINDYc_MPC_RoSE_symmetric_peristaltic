...
# from scipy import zeros, signal, random  # removed: zeros/random not in modern scipy
...
name = "TOFADCandPer"
Bolustype = "Dry"
RoSEv2pt0_TOFADCandPer = TOFandWebCam(path, "TOFADCandPer_RoSEv2pt0_40mmat20mmps", name)
RoSEv2pt0_TOFADCandPer.find_all()
# Original category in PC QSR_DoubleLayer_TOFADCandPer.FileReading([12,13,6,14,15])
RoSEv2pt0_TOFADCandPer.FileReading([0,6,5,3,1])#8,5,6,7
#12,13
dict_keys=list(RoSEv2pt0_TOFADCandPer.ArrayDict.keys())
...
"""
sample_rate = 10.0
tr_band=1 #By reducing it ,width of main lobe decreases and hence less transition captured
ripple_db = 10
cutoff_hz = 0.1
taps=TOFandWebCam.DesignFilter(sample_rate ,tr_band,ripple_db,cutoff_hz)

# ## Apply filter
# filtered_x_adc=RoSEv2pt0_DoubleLayer_TOFADCandPer.ApplyFilter(taps,x_adc['1.0'])
...
for c, value in enumerate(dict_keys):
    ...
    13       60_100_130    0170 
    """
    int_data=(170,170,170,170,170,170)#(1100,1100,1100,1100,170)
    num_of_dt_pts=(8000,8000,8000,8000,8000)#(5000,5000,5000,5000,12000)  
    
    train_window=(int_data[c],int_data[c]+num_of_dt_pts[c])
    layer_index=2  #TOF working fine for Layer 1 (less acceptable), 2,5,6,7, 8(acceptable),9,10(acceptable)
    x_tof_online_fil_list=x_tof_online_fil_list+RoSEv2pt0_TOFADCandPer.ArrayDict[dict_keys[c]]\
                                 [train_window[0]:train_window[1],
                                 [layer_index+10]].tolist()
    x_adc_list=x_adc_list+RoSEv2pt0_TOFADCandPer.ArrayDict[dict_keys[c]]\
                                 [train_window[0]:train_window[1],
                                 [layer_index+20]].tolist()
    u_data_list= u_data_list+RoSEv2pt0_TOFADCandPer.ArrayDict[dict_keys[c]]\
                                  [train_window[0]:train_window[1],
                                  [layer_index+32]].tolist()
        
    ## Apply filter
    tempList=RoSEv2pt0_TOFADCandPer.ApplyFilter(taps,RoSEv2pt0_TOFADCandPer.ArrayDict[dict_keys[c]]\
                                 [0:-1,
                                 [23]]).tolist()
...
FigureNum=plt.figure(num=1, figsize=(14, 10))
FigureNum.suptitle('Raw Sensor Data — ADC 1-State Symmetric (07/10)',
...
raw_labels  = ['DAC Input (u)', 'Raw ADC (x)', 'Filtered ADC', 'Online Filtered TOF']
raw_colors  = ['tab:orange', 'tab:blue', 'tab:green', 'tab:purple']

ax[0].plot(t_data_list[initial:final],u_data_list[initial:final],
          alpha=0.8, color=raw_colors[0],
          label=raw_labels[0])
ax[1].plot(t_data_list[initial:final],x_adc_list[initial:final],
          alpha=0.8, color=raw_colors[1],
          label=raw_labels[1])
ax[2].plot(t_data_list[initial:final][:len(x_adc_filtered_list[initial:final])],x_adc_filtered_list[initial:final],
          alpha=0.8, color=raw_colors[2],
          label=raw_labels[2])
ax[3].plot(t_data_list[initial:final][:len(x_tof_online_fil_list[initial:final])],x_tof_online_fil_list[initial:final],
          alpha=0.8, color=raw_colors[3],
          label=raw_labels[3])

for a in ax:
    a.legend(loc='upper right', fontsize=9, framealpha=0.9)
    a.grid(True, alpha=0.3)
    a.set_ylabel('ADC value', fontsize=10)
ax[-1].set_xlabel('Time (s)', fontsize=11)
FigureNum.tight_layout(rect=[0, 0, 1, 0.96]) 
...
RoSEv2pt0_Obj_SINDYc_SI=SINDYc_MPC_Design('RoSEv2pt0_SINDYc_SI',tspan)
# # =============================================================================
# # Split training and test data
# # =============================================================================

# RoSEv2pt0_Obj_SINDYc_SI.SplitData2TrainAndValid(u=u_adc_offline_filtered,
#                                                       x=x_adc_offline_filtered,
#                                                       #x=x_tof_online_fil['1.0'],
#                                                       split=0.7,selection='defined')
N_train=2200
u_RoSEv2pt0_train=np.array(u_data_list[0:N_train])
# u_RoSEv2pt0_test=RoSEv2pt0_Obj_SINDYc_SI.DataDictionary['u_valid']
u_RoSEv2pt0_all=np.array(u_data_list)
# # u_valid=  

x_RoSEv2pt0_tof_train=np.array(x_tof_online_fil_list[0:N_train])
x_RoSEv2pt0_tof_all=np.array(x_tof_online_fil_list)

# x_RoSEv2pt0_test=RoSEv2pt0_Obj_SINDYc_SI.DataDictionary['x_valid']    
# x_RoSEv2pt0_all=x_adc_offline_filtered 

x_RoSEv2pt0_adc_train=np.array(x_adc_filtered_list[0:N_train])
x_RoSEv2pt0_adc_all=np.array(x_adc_filtered_list)
                                             
tspan_train=tspan[0:N_train]
tspan_all=tspan

threshold_vec=np.array([0.002])#0.0008
...
for i in range(threshold_vec.shape[0]):
    ...
    stlsq_optimizer = ps.STLSQ(threshold=threshold_vec[i], alpha=.5)
...
# =============================================================================
    ...
    model_discrete.fit(x_RoSEv2pt0_adc_train[:-1], t=1.0, x_dot=x_RoSEv2pt0_adc_train[1:], u=u_RoSEv2pt0_train[:-1])
    model_discrete.print()
    
    # Creating a dummy test dataset from scaling train dataset
    u_RoSEv2pt0_dummy_test=np.concatenate((u_RoSEv2pt0_all,
                                                  0.8 *(u_RoSEv2pt0_train[0:400,:]-40)+40,
                                                  0.6 *(u_RoSEv2pt0_train[0:400,:]-40)+40,
                                                  0.4 *(u_RoSEv2pt0_train[0:400,:]-40)+40,
                                                  0.2 *(u_RoSEv2pt0_train[0:400,:]-40)+40,
                                                  0.1 *(u_RoSEv2pt0_train[0:400,:]-40)+40
                                                       ))
    
    N=u_RoSEv2pt0_all.shape[0]
    x0_RoSEv2pt0_adc_all=x_RoSEv2pt0_adc_all[0,np.newaxis]
    # x0_RoSEv2pt0_adc_all=x_RoSEv2pt0_adc_all[0]
    # x_RoSEv2pt0_adc_all__discrete_sim = model_discrete.simulate(x0_RoSEv2pt0_adc_all, 
    # 
    N_all=u_RoSEv2pt0_dummy_test.shape[0]
    x_RoSEv2pt0_adc_all_sim=np.zeros((N_all,1))
    x_RoSEv2pt0_adc_all_sim[0,:]=x0_RoSEv2pt0_adc_all
    """
    # instead of simulate , we can use predict directly. To use simulate use the following
    x_RoSEv2pt0_adc_all__discrete_sim[ii] = model_discrete.simulate(x0_RoSEv2pt0_adc_all,2 
    #                                              u_RoSEv2pt0_all[ii-1:ii])[1]
    """
    for ii in range(1,N_all):
        x_RoSEv2pt0_adc_all_sim[ii] = model_discrete.predict(x0_RoSEv2pt0_adc_all, 
                                                  u_RoSEv2pt0_dummy_test[ii-1:ii])
        # print(u_RoSEv2pt0_all[ii-1:ii])
        # print(x_RoSEv2pt0_adc_all_sim[ii])
        x0_RoSEv2pt0_adc_all=x_RoSEv2pt0_adc_all_sim[ii]
        
        
    FinPt1=u_RoSEv2pt0_train.shape[0]
    FinPt2=u_RoSEv2pt0_all.shape[0]
    FinPt3=u_RoSEv2pt0_dummy_test.shape[0]
    
    FigureNum=plt.figure(num=5+i, figsize=(14, 8))
    FigureNum.suptitle('DTSINDYc Model Validation — ADC 1-State Symmetric\n'
                       f'(threshold = {threshold_vec[i]:.4f})',
                       fontsize=14, fontweight='bold')
    ax3=[FigureNum.add_subplot(2,1,j) for j in range(1,3)]
    
    ax3[0].plot(np.arange(0,FinPt1),u_RoSEv2pt0_dummy_test[0:FinPt1,:],
              alpha=0.8, color='tab:red', linewidth=1.5,
              label='Input (train)')
    ax3[0].plot(np.arange(FinPt1,FinPt2),u_RoSEv2pt0_dummy_test[FinPt1:FinPt2,:],
              alpha=0.8, color='tab:green', linewidth=1.5,
              label='Input (valid)')
    ax3[0].plot(np.arange(FinPt2,FinPt3),u_RoSEv2pt0_dummy_test[FinPt2:FinPt3,:],
              alpha=0.8, color='tab:blue', linewidth=1.5,
              label='Input (dummy test)')
    ax3[0].axvline(x=FinPt1, color='grey', linestyle=':', linewidth=1)
    ax3[0].axvline(x=FinPt2, color='grey', linestyle=':', linewidth=1)
    ax3[0].set_ylabel('DAC command', fontsize=10)
    ax3[0].legend(loc='upper right', fontsize=9, framealpha=0.9, ncol=3)
    ax3[0].grid(True, alpha=0.3)
    
    for j in range (1,2):
        ax3[j].plot(np.arange(0,FinPt1),x_RoSEv2pt0_adc_all[0:FinPt1,j-1],
                    color='tab:red', linewidth=1.5, label='Measured (train)')
        _xval = x_RoSEv2pt0_adc_all[FinPt1:FinPt2,j-1]
        ax3[j].plot(np.arange(FinPt1, FinPt1 + len(_xval)),_xval,
                    color='tab:green', linewidth=1.5, label='Measured (valid)')
        ax3[j].plot(np.arange(0,FinPt3),x_RoSEv2pt0_adc_all_sim[0:FinPt3,j-1],
                       color='black', linewidth=1.5, linestyle='--', label='Model simulation')
        ax3[j].axvline(x=FinPt1, color='grey', linestyle=':', linewidth=1)
        ax3[j].axvline(x=FinPt2, color='grey', linestyle=':', linewidth=1)
        ax3[j].set_ylabel(r'State $x_1$ (ADC)', fontsize=10)
        ax3[j].legend(loc='upper right', fontsize=9, framealpha=0.9, ncol=3)
        ax3[j].grid(True, alpha=0.3)
    ax3[-1].set_xlabel('Sample index (k)', fontsize=11)
    FigureNum.tight_layout(rect=[0, 0, 1, 0.96])
    plt.show()
...
#     print('Running from the same module\n')
#     main()