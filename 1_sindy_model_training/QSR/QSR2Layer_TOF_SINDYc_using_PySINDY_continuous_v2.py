...
# from scipy import zeros, signal, random  # removed: not in modern scipy
...
QSR_DoubleLayer_TOFADCandPer.FileReading([4,0,18,2,11,17,15,10])
#12,13
...
for i in range(threshold_vec.shape[0]):
    ...
    try: 
        ...
        model.fit(x_QSR_DoubleLayer_tof_train, u=u_QSR_DoubleLayer_train, t=dt)
        model.print()
        ...
        x0_QSR_DoubleLayer_tof_all=x_QSR_DoubleLayer_tof_all[0]
        x_QSR_DoubleLayer_tof_all_sim = model.simulate(x0_QSR_DoubleLayer_tof_all, 
        ...
        ax3[0].plot(t_data_list[initial:final],u_data_list[initial:final],
                  alpha=0.7,
                  label='Input')
        ax3[1].plot(tspan_all[0:FinPt], x_QSR_DoubleLayer_tof_all[0:FinPt],#/np.max(x_QSR_DoubleLayer_tof_all[0:FinPt]),
                  'k', label='true simulation')
        ax3[1].plot(tspan_all[0:FinPt], x_QSR_DoubleLayer_tof_all_sim[0:FinPt],#/np.max(x_QSR_DoubleLayer_tof_all_sim[0:FinPt]),
