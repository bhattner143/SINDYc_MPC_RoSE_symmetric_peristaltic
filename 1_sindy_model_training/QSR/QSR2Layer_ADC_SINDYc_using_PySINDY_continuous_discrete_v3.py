...
# =============================================================================
GenPath = os.getcwd()
path = GenPath+"/DataFiles/Data02_07_2020/"
name = "TOFADCandPer"
Bolustype = "Dry"
...
QSR_DoubleLayer_TOFADCandPer.FileReading([14,12,15,1,8])
#12,13
...
for c, value in enumerate(dict_keys):
    ...
    13       60_100_130    0170 
    """
    int_data=(170,170,170,170,170)#(1100,1100,1100,1100,170)
    num_of_dt_pts=(10000,10000,10000,10000,10000)#(5000,5000,5000,5000,12000)  
    
    train_window=(int_data[c],int_data[c]+num_of_dt_pts[c])
    
    x_tof_online_fil_list=x_tof_online_fil_list+QSR_DoubleLayer_TOFADCandPer.ArrayDict[dict_keys[c]]\
                                 [train_window[0]:train_window[1],
                                 [4]].tolist()
    x_adc_list=x_adc_list+QSR_DoubleLayer_TOFADCandPer.ArrayDict[dict_keys[c]]\
                                 [train_window[0]:train_window[1],
                                 [8]].tolist()
    u_data_list= u_data_list+QSR_DoubleLayer_TOFADCandPer.ArrayDict[dict_keys[c]]\
                                  [train_window[0]:train_window[1],
                                  [20]].tolist()
        
    ## Apply filter
    tempList=QSR_DoubleLayer_TOFADCandPer.ApplyFilter(taps,QSR_DoubleLayer_TOFADCandPer.ArrayDict[dict_keys[c]]\
                                 [0:-1,
                                 [8]]).tolist()
...
u_QSR_DoubleLayer_train=np.array(u_data_list[0:5000])
# u_QSR_DoubleLayer_test=QSR_DoubleLayer_Obj_SINDYc_SI.DataDictionary['u_valid']
u_QSR_DoubleLayer_all=np.array(u_data_list)
# # u_valid=  

x_QSR_DoubleLayer_tof_train=np.array(x_tof_online_fil_list[0:5000])
x_QSR_DoubleLayer_tof_all=np.array(x_tof_online_fil_list)

# x_QSR_DoubleLayer_test=QSR_DoubleLayer_Obj_SINDYc_SI.DataDictionary['x_valid']    
# x_QSR_DoubleLayer_all=x_adc_offline_filtered 

x_QSR_DoubleLayer_adc_train=np.array(x_adc_filtered_list[0:5000])
x_QSR_DoubleLayer_adc_all=np.array(x_adc_filtered_list)
                                             
tspan_train=tspan[0:5000]
tspan_all=tspan

threshold_vec=np.array([0.00006])
...
for i in range(threshold_vec.shape[0]):
    ...
    model.fit(x_QSR_DoubleLayer_adc_train, u=u_QSR_DoubleLayer_train, t=dt)
    model.print()
    ...
    FinPt=u_QSR_DoubleLayer_all.shape[0]-1000
    x0_QSR_DoubleLayer_adc_all=x_QSR_DoubleLayer_adc_all[0]
    x_QSR_DoubleLayer_adc_all_sim = model.simulate(x0_QSR_DoubleLayer_adc_all, 
    ...
    ax3[0].plot(t_data_list[initial:final],u_data_list[initial:final],
              alpha=0.7,
              label='Input')
    ax3[1].plot(tspan_all[0:FinPt], x_QSR_DoubleLayer_adc_all[0:FinPt],#/np.max(x_QSR_DoubleLayer_tof_all[0:FinPt]),
              'k', label='true simulation')
    ax3[1].plot(tspan_all[0:FinPt], x_QSR_DoubleLayer_adc_all_sim[0:FinPt],#/np.max(x_QSR_DoubleLayer_tof_all_sim[0:FinPt]),
...
# =============================================================================
    ...
    model_discrete.fit(x_QSR_DoubleLayer_adc_train[:-1], t=1.0, x_dot=x_QSR_DoubleLayer_adc_train[1:], u=u_QSR_DoubleLayer_train[:-1])
    model_discrete.print()

    
    N=tspan_all[0:FinPt].shape[0]
    x0_QSR_DoubleLayer_adc_all=x_QSR_DoubleLayer_adc_all[0]
    x_QSR_DoubleLayer_adc_all__discrete_sim = model_discrete.simulate(x0_QSR_DoubleLayer_adc_all, 
                                                  N, u_QSR_DoubleLayer_all)
    ...
    ax3[0].plot(t_data_list[initial:final],u_data_list[initial:final],
              alpha=0.7,
              label='Input')
    ax3[1].plot(x_QSR_DoubleLayer_adc_all[0:FinPt],#/np.max(x_QSR_DoubleLayer_tof_all[0:FinPt]),
              'k', label='true simulation')
    ax3[1].plot(x_QSR_DoubleLayer_adc_all__discrete_sim[0:FinPt],#/np.max(x_QSR_DoubleLayer_tof_all_sim[0:FinPt]),
                  'r-', label='model simulation')
    plt.show()
