...
#        pdb.set_trace()
    ...
    def GenerateRobotSINDYcModel(self,
        ...
        if discrete_model is True:
            ...
            # PySINDy 2.x: discrete-time via x_dot=x[1:] (finite diff trick)
            self.model = ps.SINDy(optimizer=stlsq_optimizer,
                feature_library=library_sindyc
                #differentiation_method=smoothedFD
                )
            
            self.model.fit(x_robot_adc_train[:-1], t=1.0,
                           x_dot=x_robot_adc_train[1:], u=u_robot_train[:-1])
...
def main():
    ...
    #                pdb.set_trace()
            ...
            for idx in range(3):
                ...
                ax.set_ylabel(ctrl_labels[idx])
                ax.set_ylim(0, 125)
                ax.legend(loc='upper right', framealpha=0.9)
                ax.grid(True, alpha=0.3)

            f1.axes[-1].set_xlabel('Time step (k)', fontsize=12)
            f1.suptitle('MPC Simulation — DTSINDYc Model as Plant (3-State Peristalsis)',
