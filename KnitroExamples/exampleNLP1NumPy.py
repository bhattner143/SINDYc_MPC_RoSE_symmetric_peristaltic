...
n = 2
KN_add_vars (kc, n)
KN_set_var_lobnds (kc, xLoBnds = np.array ([-KN_INFINITY, -KN_INFINITY])) # not necessary since infinite
KN_set_var_upbnds (kc, xUpBnds = np.array ([0.5, KN_INFINITY]))
# Define an initial point.  If not set, Knitro will generate one.
KN_set_var_primal_init_values (kc, xInitVals = np.array ([-2.0, 1.0]))

# Add the constraints and set their lower bounds
m = 2
KN_add_cons(kc, m)
KN_set_con_lobnds (kc, cLoBnds = np.array ([1.0, 0.0]))
