...
class RoSE_actuation_protocol():
    ...
    def __init__(self,RPi_Cobbler_Pin_No=18,
        ...
        if UseIOExpander is True:
            ...
            self.InitializeIOExpanderAndChangeTOFAddress((0x40,0x41))
        else:
...
##        ax2.set_ylim([40,45])
    ...
    @staticmethod
    def Filter_RealTime_design(numtaps=200,cutoff=0.001):
    ...
    @staticmethod
    def Filter_RealTime_apply(actual_data,b,z,size_data=[],n=2):
...
##                                   actual_data.shape[0]))
##            #for i, x in enumerate(actual_data[:,1:]):
##            _, z1 = signal.lfilter(b, 1, [actual_data[0]], zi=z[0])
##            filtered_data[0,0],z1 = signal.lfilter(b, 1, [actual_data[0]], zi=z1)
##            
##            _, z2 = signal.lfilter(b, 1, [actual_data[1]], zi=z[1])
##            filtered_data[0,1], z2 = signal.lfilter(b, 1, [actual_data[1]], zi=z2)
##            
##            return filtered_data, (z1,z2)
        ...
        if n==1:
            ...
            if size_data==[]:
                ...
                filtered_data, z=RoSE_actuation_protocol.Filter_RealTime_apply(filtered_data[0,:],b,(z1,z2),size_data=[],n=n-1)
...
##            dfHex=[hex(df[j,x]) for x in range(12)]
    ...
    # Generate a csv file from the TOF 2d data
        if QSR_Two_layer.Flag_UseIOExpander is True and QSR_Two_layer.Flag_UseADC is False:
            ...
            RoSE_actuation_protocol.TOF_Plottting(Figure1, TOFADCPer2dArray[:,[0,1,2]],TOFADCPer2dArray[:,[0,3,4]])
