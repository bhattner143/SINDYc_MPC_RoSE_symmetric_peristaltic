...
# Original category in PC QSR_DoubleLayer_Obj_SINDYc_SI.FileReading([12,13,6,14,15])
QSR_DoubleLayer_Obj_SINDYc_SI.FileReading([12,13,6,14,15,16,17])
...
Index    File          int_data   num_of_dt_pt
    ...
    13       60_100_130    0170 
"""             
int_data=(170,170,170,170,170,170,170)#(1100,1100,1100,1100,170)
...
x_robot_adc_train=QSR_DoubleLayer_Obj_SINDYc_SI.DataDictionary['x_train'][:,0]
x_robot_adc_test=QSR_DoubleLayer_Obj_SINDYc_SI.DataDictionary['x_valid'][:,0]
...
for i in range(threshold_vec.shape[0]):
    ...
    try: 
        ...
        model.fit(x_robot_adc_train, u=u_robot_train, t=dt)
        model.print()
        ...
        u_interp_robot_valid = interp1d(tspan_robot_valid, 
                                                  u_robot_valid, 
                                                  axis=0, kind='cubic')
        ...
        x0_robot_adc_valid=x_robot_adc_valid[0]
        x_robot_adc_valid_sim = model.simulate(x0_robot_adc_valid, 
                                                      tspan_robot_valid[0:FinPt], 
                                                      u_interp_robot_valid)
        ...
        ax3[0].plot(t_data_list[initial:final],u_data_list[initial:final],
                  alpha=0.7,
                  label='Input')
        ax3[1].plot(tspan_robot_valid[0:FinPt], x_robot_adc_valid_sim[0:FinPt],#/np.max(x_QSR_DoubleLayer_tof_all[0:FinPt]),
                  'k', label='true simulation')
        ax3[1].plot(tspan_robot_valid[0:FinPt], x_robot_adc_valid_sim[0:FinPt],#/np.max(x_QSR_DoubleLayer_tof_all_sim[0:FinPt]),
...
# u_control=np.concatenate((np.concatenate((55*np.ones(100),
#                                           100*np.ones(155))),75*np.ones(125)))
xk=x0_robot_adc_valid
xk1_list=[]
for ii in range(0,u_control.shape[0]):
    xk1=model.simulate(xk, u_control[ii])
...
ax3[0].plot(u_control,
          alpha=0.7,
          label='Input')
ax3[1].plot(xk1_list,#/np.max(x_QSR_DoubleLayer_tof_all[0:FinPt]),
      'k', label='prediction')
plt.show()