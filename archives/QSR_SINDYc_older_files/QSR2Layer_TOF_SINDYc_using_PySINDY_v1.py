...
from archives.tvregdiff_master.tvregdiff import TVRegDiff
...
ax[1].plot(t_data['1.0'][initial:final],x_adc['1.0'][initial:final,0],
          alpha=0.7,
          label='ADC')
ax[2].plot(t_data['1.0'][initial+200:final],filtered_x_tof[initial+200:final,0],
          alpha=0.7,
          label='Filtered TOF')
...
x_tof_offline_filtered=filtered_x_tof[200:final,0]
x_tof_offline_filtered=x_tof_offline_filtered[...,np.newaxis]
u_tof_offline_filtered=u_data['1.0'][200:final,0]
# =============================================================================
...
QSR_DoubleLayer_Obj_SINDYc_SI.SplitData2TrainAndValid(u=u_tof_offline_filtered,
                                                      x=x_tof_offline_filtered,
...
u_QSR_DoubleLayer_all=u_tof_offline_filtered
# u_valid=  
...
x_QSR_DoubleLayer_all=x_tof_offline_filtered 
                                             
tspan_train=tspan[0:x_QSR_DoubleLayer_train.shape[0]]
tspan_all=tspan[0:x_QSR_DoubleLayer_all.shape[0]]
...
stlsq_optimizer = ps.STLSQ(threshold=.0001, alpha=.5,normalize=False)

fourier_library = ps.FourierLibrary(n_frequencies=3)
poly_library = ps.PolynomialLibrary(include_interaction=True,degree=3)
...
model.fit(x_QSR_DoubleLayer_train, u=u_QSR_DoubleLayer_train, t=dt)
model.print()

# =============================================================================
# Compare SINDy-predicted derivatives with finite difference derivatives
# =============================================================================
#Comparison on the training data
tspan_valid = tspan_train
x_QSR_DoubleLayer_valid= x_QSR_DoubleLayer_train
u_QSR_DoubleLayer_valid = u_QSR_DoubleLayer_train

# 
# print('Model score: %f' % model.score(x_QSR_DoubleLayer_valid, u_QSR_DoubleLayer_valid, t=dt))

# Predict derivatives using the learned model
x_dot_QSR_DoubleLayer_test_predicted = model.predict(x_QSR_DoubleLayer_valid, u=u_QSR_DoubleLayer_valid)  

# Compute derivatives with a finite difference method, for comparison
x_dot_QSR_DoubleLayer_test_computed = model.differentiate(x_QSR_DoubleLayer_valid, t=dt)

fig, axs = plt.subplots(x_QSR_DoubleLayer_valid.shape[1], 1, sharex=True, figsize=(2.5, 1.5))
for i in range(x_QSR_DoubleLayer_valid.shape[1]):
    axs.plot(tspan_valid , x_dot_QSR_DoubleLayer_test_computed[:, i],
                'k', label='numerical derivative')
    axs.plot(tspan_valid , x_dot_QSR_DoubleLayer_test_predicted[:, i],
                'r--', label='model prediction')
    axs.legend()
    axs.set(xlabel='t', ylabel='$\dot x_{}$'.format(i))
fig.show()
...
u_interp_QSR_DoubleLayer_all = interp1d(tspan_all, 
                                          u_QSR_DoubleLayer_all, 
                                          axis=0, kind='cubic')
...
FinPt=13500
x0_QSR_DoubleLayer_all=x_QSR_DoubleLayer_all[0]
x_QSR_DoubleLayer_all_sim = model.simulate(x0_QSR_DoubleLayer_all, 
                                             tspan_all[0:FinPt], u_interp_QSR_DoubleLayer_all)

# fig, axs = plt.subplots(x_QSR_DoubleLayer_all.shape[1], 1, sharex=True, figsize=(3.5, 4.5))
# for i in range(x_QSR_DoubleLayer_all.shape[1]):
#     axs.plot(tspan_all[0:FinPt], x_QSR_DoubleLayer_all[0:FinPt]/np.max(x_QSR_DoubleLayer_all[0:FinPt]), 'k', label='true simulation')
#     axs.plot(tspan_all[0:FinPt], x_QSR_DoubleLayer_all_sim[0:FinPt]/np.max(x_QSR_DoubleLayer_all_sim[0:FinPt]), 'r--', label='model simulation')
#     axs.legend()
#     axs.set(xlabel='t', ylabel='$x_{}$'.format(i))
    
FigureNum=plt.figure(num=4, figsize=(2.5, 1.5))
ax3=[FigureNum.add_subplot(2,1,i) for i in range(1,3)]

ax3[0].plot(t_data['1.0'][initial:final],u_data['1.0'][initial:final,0],
          alpha=0.7,
          label='Input')
ax3[1].plot(tspan_all[0:FinPt], x_QSR_DoubleLayer_all[0:FinPt]/np.max(x_QSR_DoubleLayer_all[0:FinPt]),
         'k', label='true simulation')
ax3[1].plot(tspan_all[0:FinPt], x_QSR_DoubleLayer_all_sim[0:FinPt]/np.max(x_QSR_DoubleLayer_all_sim[0:FinPt]),
