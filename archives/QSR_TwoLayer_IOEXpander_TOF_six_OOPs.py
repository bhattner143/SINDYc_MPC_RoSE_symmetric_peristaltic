...
class RoSE_actuation_protocol():
    ...
    def __init__(self,RPi_Cobbler_Pin_No=18,
        ...
        if UseIOExpander is True:
            ...
            self.InitializeIOExpanderAndChangeTOFAddress((0x40,0x41,0x42,0x43,0x44,0x45))
        else:
...
#==============================================================================
    def GenerateDisplacementDataFromTOF(self,t1):  
        return np.array([self.GenerateTimestamp(t1),
                         self.TOF_sensor[0].range,self.TOF_sensor[0].range,self.TOF_sensor[0].range,self.TOF_sensor[0].range,self.TOF_sensor[0].range,
                         self.TOF_sensor[1].range,self.TOF_sensor[1].range,self.TOF_sensor[1].range,self.TOF_sensor[1].range,self.TOF_sensor[1].range,
                         self.TOF_sensor[2].range,self.TOF_sensor[2].range,self.TOF_sensor[2].range,self.TOF_sensor[2].range,self.TOF_sensor[2].range,
                         self.TOF_sensor[3].range,self.TOF_sensor[3].range,self.TOF_sensor[3].range,self.TOF_sensor[3].range,self.TOF_sensor[3].range,
                         self.TOF_sensor[4].range,self.TOF_sensor[4].range,self.TOF_sensor[4].range,self.TOF_sensor[4].range,self.TOF_sensor[4].range,
                         self.TOF_sensor[5].range,self.TOF_sensor[5].range,self.TOF_sensor[5].range,self.TOF_sensor[5].range,self.TOF_sensor[5].range])
...
#==============================================================================
    ...
    ##                    'Sample 2_5 (mm)'
            ...
            filename2="CsvData22_07_2020/QSR_2Layer_"+DataFrom+"_test_"+str(ScalingFactor)+"_"+\
                       filenameForSaving[0:]+".csv"
...
##        ax2.set_ylim([40,45])
    ...
    @staticmethod
    def Filter_RealTime_apply(actual_data,b,z,size_data=[],n=6):
...
##            dfHex=[hex(df[j,x]) for x in range(12)]
            ...
            QSR_Two_layer.mergeDACadd2DataAndSend(dfFlipped,j,BaseLinePress,ScalingFact,size_df[1],40)
            ...
            if QSR_Two_layer.Flag_UseIOExpander is True and QSR_Two_layer.Flag_UseADC is False:
                ...
                range_pressure_array=np.hstack((np.hstack((range_mm_array_mean,
                           ...
                           pressure_kpa_array))
                range_pressure_peristalsis_array=np.hstack((range_pressure_array,dfFlipped[j,:]))
    ...
    # Generate a csv file from the TOF 2d data
        if QSR_Two_layer.Flag_UseIOExpander is True and QSR_Two_layer.Flag_UseADC is False:
            ...
            RoSE_actuation_protocol.TOF_Plottting(Figure1, TOFADCPer2dArray[:,[0,1,2,3,4,5,6]],TOFADCPer2dArray[:,[0,7,8,9,10,11,12]])
        ...
        port_a_pins[1].value = True
        port_a_pins[2].value = True
        port_a_pins[3].value = True
        port_a_pins[4].value = True
        port_a_pins[5].value = True
