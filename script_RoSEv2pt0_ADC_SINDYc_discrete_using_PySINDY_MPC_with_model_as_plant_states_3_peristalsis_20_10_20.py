...
from lib.sindybase import *
from lib.ClassTOFandWebCam import *

from itertools import count
from matplotlib.animation import FuncAnimation

# =============================================================================
# USE_HARDWARE flag: set True when running on Raspberry Pi with RoSE hardware,
# set False for offline model-as-plant simulation
# =============================================================================
USE_HARDWARE = False

if USE_HARDWARE:
    from lib.RoSE_IOEXpander_TOF_cal_ten_OOPs import*
else:
    class RoSE_actuation_protocol:
        """Stub for offline model-as-plant simulation (no hardware required)."""
        def __init__(self, **kwargs):
            self.Flag_UseIOExpander = False
            self.Flag_UseADC = False
        @staticmethod
        def Filter_RealTime_design(numtaps=50):
            import scipy.signal as _sig
            b = _sig.firwin(numtaps, 0.1)
            z = _sig.lfilter_zi(b, [1.0])
            return b, z
        @staticmethod
        def Filter_RealTime_apply(x, b, z):
            import scipy.signal as _sig
            return _sig.lfilter(b, [1.0], x, zi=z), z
        def mergeDACadd2DataAndSend(self, *args, **kwargs):
            pass
        def GenerateTimestamp(self, **kwargs):
            return 0.0
        def GeneratePressureDataFromADC(self, **kwargs):
            return np.zeros(12)
        def GenerateDisplacementDataFromTOF(self, *args):
            return np.zeros(11)
...
#        pdb.set_trace()
    ...
    def GenerateRobotSINDYcModel(self,
        ...
        if discrete_model is True:
            ...
            # PySINDy 2.x discrete-time: fit x_{k+1} = f(x_k, u_k) by passing
            # x_dot = x[1:] (next states) so predict() returns x_{k+1} directly
            self.model = ps.SINDy(optimizer=stlsq_optimizer,
                feature_library=library_sindyc
                )
            self.model.fit(x_robot_adc_train[:-1],
                           t=1.0,
                           x_dot=x_robot_adc_train[1:],
                           u=u_robot_train[:-1])
    ...
    def SimulateRobotSINDYcModel(self,x0,N,u):
        ...
        t = np.arange(N+1) * 0.1
        x_sindyc_model_sim = self.model.simulate(x0, t, u=u)
    ...
    #        Obtain plant state at next prediction step
            ...
            xk1_model = self.model.predict(xk_model, u=uk_model)  # returns x_{k+1} directly
            xk1=xk1_model[0,:]
...
try:
    ...
    for i in range(Nvec.shape[0]):
        ...
        Duration    = 120#43              # Run control for 100 time units
    ...
    #    D = 0                          # Feedforward (none)
        ...
        xref_df_all=pd.read_csv('ControllerReferenceFiles/Reference_traj_de_simulated_2.csv')
        ...
        xref_traj=np.concatenate((xref0.T,xref1.T,xref2.T,xref3.T)).T
...
#                    pdb.set_trace()
                ################################    
                if xref[0]>=72 and xref[0]<74:
                    LB1=40 #Lower bound
                    UB1=44 #Upper bound
                    
                elif xref[0]>=70 and xref[0]<72:
                    LB1=34 #Lower bound
                    UB1=36 #Upper bound
                    
                elif xref[0]>=68 and xref[0]<70:
                    LB1=25 #Lower bound
                    UB1=30 #Upper bound
                    
                elif xref[0]>=67 and xref[0]<68:
                    LB1=23 #Lower bound
                    UB1=25 #Upper bound
                else:
                    LB1=9
                    UB1=17
                #########################    
                if xref[1]>=71 and xref[0]<74:
                    LB1=40 #Lower bound
                    UB1=44 #Upper bound
                    
                elif xref[1]>=70 and xref[1]<71:
                    LB1=34 #Lower bound
                    UB1=36 #Upper bound
...
#                elif xref[1]>=65.9 and xref[1]<70.9:
#                    LB2=25
#                    UB2=30
#                elif xref[1]>=67.7 and xref[1]<70.9:
#                    LB2=35
#                    UB2=37
                else:
                    LB2=9
                    UB2=17
                 ###################################  
                if xref[2]>=72 and xref[2]<74:
                    LB1=40 #Lower bound
                    UB1=44 #Upper bound                    
                elif xref[2]>=70 and xref[2]<72:
                    LB1=34 #Lower bound
                    UB1=36 #Upper bound
                    ...
                    LB3=20 #Lower bound
                    UB3=25 #Upper bound
#                elif xref[2]>=65.4 and xref[2]<72.9:
#                    LB3=17
#                    UB3=38
#                elif xref[2]>=68.2 and xref[2]<70.9:
#                    LB3=28
#                    UB3=30
#                elif xref[2]>=70.9 and xref[2]<72.9:
#                    LB3=35
#                    UB3=37
                else:
                    LB3=9
            ...
            if USE_HARDWARE:
                #Apply input to RoSE
                uopt0_all_layers=uopt[0,0].T*np.ones((3,num_rose_layers))+2
            ...
            #Generate ADC/TOF output from RoSE or simulate model
            ...
            and RoSEv2pt0_Obj_SINDYc_SI.Flag_UseADC is True:
                ...
                x = np.array([pressure_kpa_array[4:7]])
                x[0,0]=x[0,0]-10
                x[0,1:3]=x[0,1:3]-5
                ...
                # --- Model-as-plant: simulate one step with SINDYc model ---
                assert x_model.shape[0]==1, 'x_model must have shape (1, n_states)'
                uopt_model=uopt[:,0]
                uopt_model=uopt_model[:,np.newaxis].T
                x = RoSEv2pt0_Obj_SINDYc_SI.model.predict(x_model, u=uopt_model)  # returns x_{k+1}
                x_model = x  # update for next iteration

                t_fi=np.array([ct * Ts])

                x_tof=x.copy()
                x_tof_filtered=x.copy()
        ...
        state_labels = [r'$x_1$ (ADC Layer 0)', r'$x_2$ (ADC Layer 1)', r'$x_3$ (ADC Layer 2)']
        ctrl_labels  = [r'$u_1$ (DAC Layer 0)', r'$u_2$ (DAC Layer 1)', r'$u_3$ (DAC Layer 2)']
        state_colors = ['tab:red', 'tab:green', 'tab:blue']
        ref_colors   = ['salmon', 'lightgreen', 'lightskyblue']

        f1 = plt.figure(num=7, figsize=(14, 12))
        gs = f1.add_gridspec(6, 1, hspace=0.35)

        # ---------- states (rows 0-2) ----------
        for idx in range(3):
            ax = f1.add_subplot(gs[idx])
            ax.plot(xref_traj.T[:, idx], color=ref_colors[idx],
                    linewidth=2.0, linestyle='--', label='Reference')
            ax.plot(xHistory[:, idx], color=state_colors[idx],
                    linewidth=1.8, linestyle='-', label='SINDYc output')
            ax.set_ylabel(state_labels[idx], fontsize=11)
            ax.set_ylim(60, 80)
        ...
        # ---------- controls (rows 3-5) ----------
        for idx in range(3):
            ax = f1.add_subplot(gs[3 + idx])
            ax.plot(uHistory[:, idx], color=state_colors[idx],
                    linewidth=1.8, label=ctrl_labels[idx])
            ax.set_ylabel(ctrl_labels[idx], fontsize=11)
            ax.set_ylim(0, 60)
            ax.legend(loc='upper right', framealpha=0.9, fontsize=9)
            ax.grid(True, alpha=0.3)

        f1.axes[-1].set_xlabel('Time step (k)', fontsize=12)
        f1.suptitle('MPC Simulation — DTSINDYc Model as Plant\n(3-State Peristalsis, Dataset 20/10/20)',
